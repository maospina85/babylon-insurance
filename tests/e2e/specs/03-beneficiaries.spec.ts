...
// Helper: activate 'Vida / Fallecimiento' module (has beneficiaries)
...
test.describe('Suite 3 — Beneficiarios', () => {
  ...
  test('3.2 — Agregar beneficiario agrega una nueva fila', async ({ page }) => {
    ...
    const nameInputs = article.locator('[id^="ben-name-death-"]');
  ...
  test('3.4 — Nombre inválido muestra error de validación', async ({ page }) => {
    ...
    const nameInput = page.locator('[id^="ben-name-death-"]').first();
  ...
  test('3.5 — Porcentaje inválido (>100) muestra error', async ({ page }) => {
    ...
    const pctInput = page.locator('[id^="ben-pct-death-"]').first();
  ...
  test('3.6 — Dos beneficiarios 50/50 validan correctamente', async ({ page }) => {
    ...
    const nameInputs = article.locator('[id^="ben-name-death-"]');
    const relInputs  = article.locator('[id^="ben-rel-death-"]');
    const pctInputs  = article.locator('[id^="ben-pct-death-"]');
    ...
    await expect(article.getByRole('alert')).not.toBeVisible();
  });
  ...
  test('3.8 — Con suma < 100% el CTA queda deshabilitado y muestra alerta', async ({ page }) => {
    ...
    await expect(page.getByRole('alert')).toContainText('Completa los datos de beneficiarios (100%) antes de continuar.');
